...
export function ImageGenerationSection() {
  ...
  const generationSteps: GenerationStep[] = [
    {
      alt: t("pages.landing.imageGeneration.step1Alt"),
      imageSrc: "/image-generation/1.png",
      prompt: t("pages.landing.imageGeneration.step1Prompt"),
    },
    {
      alt: t("pages.landing.imageGeneration.step2Alt"),
      imageSrc: "/image-generation/2.png",
      prompt: t("pages.landing.imageGeneration.step2Prompt"),
    },
    {
      alt: t("pages.landing.imageGeneration.step3Alt"),
      imageSrc: "/image-generation/3.png",
      prompt: t("pages.landing.imageGeneration.step3Prompt"),
    },
  ]
  ...
  const currentStep = generationSteps[activeIndex]
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      setTypedPrompt("")
      setActiveIndex((previous) => (previous + 1) % generationSteps.length)
      setPhase("typing")
    }, prefersReducedMotion ? 0 : Math.round(IMAGE_FADE_DURATION_SECONDS * 1000))

    return () => window.clearTimeout(timeoutId)
  }, [generationSteps.length, phase, prefersReducedMotion])
            ...
            <div className="absolute inset-x-4 bottom-4 z-10 md:inset-x-5 md:bottom-5">
              <div className="rounded-xl border border-border/60 bg-background/70 p-3 md:p-4">
                <div className="flex items-start gap-3">
                  ...
                  <p className="min-h-12 text-start text-sm leading-6 text-foreground md:text-base">
                    {typedPrompt || "\u00A0"}
                    <span
                      className={
                        phase === "typing" ? "ms-0.5 inline-block animate-pulse" : "ms-0.5 hidden"
                      }
                    >
                      |
                    </span>
